			...
			return {
				...
				link: function ($scope, elm, attrs, ngModel) {
					var expression, options, tinyInstance,
						...
						getOption = function (mentionUser) {
							var users = [];
							if (mentionUser) {
								for (var i in mentionUser) {
									users.push({
										'id': mentionUser[i].ID,
										'name': mentionUser[i].name
									});
								}
							}
							options = {
									...
									var args,
										...
										cleanUpAndSetTask = function (element) {
											...
											if ($element.hasClass('task')) {
												...
												$rootScope.addedTaskOwners = mentionUser;
												$rootScope.addedTaskOwner = mentionUser[0];
